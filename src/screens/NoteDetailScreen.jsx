...
export default function NoteDetailScreen({ note: init, onBack, onUpdate, onDelete, dark, t, folders, subfolders = {} }) {
  ...
  const isGradient = noteBackground?.startsWith('linear');
  let bgStyle;
  if (dark || !noteBackground) {
    bgStyle = { background: t.card };
  } else if (sceneUri) {
    bgStyle = isGradient
      ? { backgroundImage: `${sceneUri}, ${noteBackground}`, backgroundSize: 'cover, cover', backgroundRepeat: 'no-repeat, no-repeat', backgroundPosition: 'center top, center' }
      : { backgroundColor: noteBackground, backgroundImage: sceneUri, backgroundSize: 'cover', backgroundRepeat: 'no-repeat', backgroundPosition: 'center top' };
  } else {
    bgStyle = { background: noteBackground };
  }
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', ...bgStyle, transition: 'background-color .3s' }}>
